function processImage() {
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            ...
            const activities = activity[Math.floor(Math.random() * activity.length)]; // Replace with dynamic data if needed
            ...
            const link = activities.replace(/ /g, '') + '.html';
            newRow.innerHTML = `
                <td>${uploaderName}</td>
                <td>${uploadReferenceID}</td>
                <td><a href="${link}" target="_blank">${activities}</a></td>
                <td>${percent}</td>
                <td>${uploadImage}</td>
                <td>${size}</td>
                <td>${mimeType}</td>
